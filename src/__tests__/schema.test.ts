...
describe('supabase-schema.sql (v2 security invariants)', () => {
  ...
  it('has no recursive app_roles subquery inside any policy body', () => {
    ...
    for (const segment of segments) {
      ...
      // No policy may reference app_roles in FROM position at all —
      // aliases or casing must not dodge the tripwire.
      expect(statement).not.toMatch(/from\s+(public\.)?app_roles\b/)
    }
  })
})
